...
関数定義のうち、パラメータリストまでの部分は関数宣言となる。関数宣言を「;」で終了させた文は、関数プロトタイプとなる。

関数プロトタイプは、コンパイラに対して、ソースコード上でそれ以降に出現した同名の関数の呼び出しについてパラメータの整合性（数、型）と、返り値の整合性を検証するように要求する。

これらの検証は、バグを持たないプログラムを作成するためには極めて重要である。したがって、常に関数プロトタイプを記述するようにする必要がある。

なお、独立した関数プロトタイプを用意しなくても、ソースコード上で関数定義より後ろにある該当の関数の呼び出しについては同じ検証が行われる。

- EX9.2:関数宣言
次の３つのソースコードは実行時の機能は全く同じであるが、コンパイル時の検証の有無が異なる
- ch09-06.c
```
#include <stdio.h>
